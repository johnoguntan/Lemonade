...
import type {
  ...
  Preset,
  PresetWithCollections,
  Profile,
} from "@/lib/types"

export interface AllsenadroStore {
  userId: string | null
  profile: Profile | null
  setUserId: (id: string | null) => void
  setProfile: (profile: Profile | null) => void

  uiPreferences: AllsenadroUIPreferences
  setUIPreferences: (prefs: Partial<AllsenadroUIPreferences>) => void
  ...
  addCollection: (collection: CollectionInsert) => Promise<void>
  ...
  addPreset: (name: string, collectionIds: string[]) => Promise<void>
  ...
  fetchJournalEntry: (date: string) => Promise<void>
  saveJournalEntry: (content: string, date: string) => Promise<void>

  notificationSettings: NotificationSettings | null
  fetchNotificationSettings: () => Promise<void>
  updateNotificationSettings: (updates: NotificationSettingsUpdate) => Promise<void>

  initializeAllsenadro: (userId: string) => Promise<void>
}

type PresetCollectionJoin = {
  sort_order: number
  collections: Collection | null
}

type PresetRowWithJoin = Preset & {
  preset_collections?: PresetCollectionJoin[] | null
}

type HabitRowWithLogs = Habit & {
  habit_logs?: Array<{
    id: string
    completed: boolean
  }> | null
...
const isLocalHabitId = (id: string) => id.startsWith("local-habit-")
const mergeLocalFallbackHabits = (existing: HabitWithLog[], incoming: HabitWithLog[]) => {
...
export const useAllsenadroStore = create<AllsenadroStore>()(
  persist(
    (set, get) => ({
      userId: null,
      profile: null,
      ...
      fetchPresets: async () => {
        ...
        const presets = ((data ?? []) as unknown as PresetRowWithJoin[]).map((preset) => ({
          id: preset.id,
      ...
      fetchHabits: async () => {
        ...
        const habits = ((data ?? []) as unknown as HabitRowWithLogs[]).map((habit) => {
      ...
      fetchJournalEntry: async (date) => {
        ...
        set({ journalEntry: data as JournalEntry })
      },
      ...
      fetchNotificationSettings: async () => {
        ...
        set({ notificationSettings: (data as NotificationSettings | null) ?? null })
      },
