...
import { detectFallbackDate } from "@/lib/task-shortcuts"
...
export const parseFlexibleDate = (value: string): Date | null => {
  ...
  const relative = detectFallbackDate(lower, formatLocalDateKey(new Date()))
  if (relative) {
    const [y, m, d] = relative.split("-").map(Number)
    if (y && m && d) return makeDate(y, m - 1, d)
  }
  ...
  if (!Number.isNaN(nativeNormalized.getTime())) return nativeNormalized
