...
export function RepeatPicker({ value, onChange, onClose }: RepeatPickerProps) {
  const frequency: RepeatFrequency = value?.frequency ?? "daily"
  const interval = value?.interval ?? 1
  const daysOfWeek = value?.daysOfWeek ?? [1]

  const commit = (updates: Partial<{ frequency: RepeatFrequency; interval: number; daysOfWeek: number[] }>) => {
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          {([
            ...
            >
              <span>{label}</span>
              {value && frequency === key ? <span>✓</span> : null}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-3 border-t border-gray-100 pt-4">
          <label className="flex items-center gap-3 text-sm text-gray-700">
            ...
            <input
              ...
              min={1}
              value={interval}
              onChange={(event) => commit({ interval: Number.parseInt(event.target.value || "1", 10) })}
        ...
        <div className="flex justify-end gap-3 pt-2">
          ...
          <button type="button" onClick={() => onClose?.()} className="rounded-xl bg-[#ff4d57] px-4 py-2 text-sm text-white">
            OK
          </button>
        </div>
      </div>
