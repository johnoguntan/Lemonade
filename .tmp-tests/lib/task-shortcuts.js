...
exports.normalizeIsoDate = exports.buildHeuristicParsedTask = exports.cleanShortcutTitle = exports.extractExplicitSignals = exports.detectImplicitTaskPriority = exports.detectRecurringPattern = exports.detectFallbackTime = exports.detectFallbackDate = exports.parseMinutesValue = exports.resolveParsedRecurringTodoFields = exports.hasShortcutFields = void 0;
...
const hasShortcutFields = (fields) => Boolean(fields.phone ||
    fields.url ||
    fields.location ||
    ...
    fields.notes ||
    fields.attachment);
exports.hasShortcutFields = hasShortcutFields;
...
const coerceRecurringDay = (value, recurring) => {
    if (typeof value !== "number" || !Number.isFinite(value))
        return null;
    const normalized = Math.floor(value);
    if (recurring === "weekly") {
...
const coerceRecurringDays = (value) => {
...
const coerceRecurringValue = (value) => {
    if (value === "daily" ||
        value === "weekday" ||
        value === "weekly" ||
        value === "monthly" ||
...
const getRecurringWeekdayIndexes = (input) => {
    const normalized = input.toLowerCase();
    const weekdayListMatch = normalized.match(new RegExp(`\\b(?:every|each)\\s+([a-z,\\s]+?)(?:\\s+at\\b|[.!?;:]|$)`, "i"));
    if (!weekdayListMatch)
        return [];
    const weekdays = Array.from(weekdayListMatch[1].matchAll(new RegExp(`\\b(${WEEKDAY_NAMES_PATTERN})\\b`, "gi")))
        .map((match) => normalizeWeekdayIndex(match[1]))
        .filter((day) => day !== null);
    return Array.from(new Set(weekdays));
};
const resolveParsedRecurringTodoFields = (task) => {
    ...
    const existingCustomText = typeof task.recurringCustomText === "string" && task.recurringCustomText.trim().length > 0
        ? task.recurringCustomText.trim()
        : undefined;
    const weeklyDayLabel = recurring === "weekly" && recurringDay !== null ? WEEKDAY_NAME_BY_INDEX[recurringDay] : null;
    const recurringFrequency = recurring === "daily" || recurring === "weekday" || recurring === "weekly" || recurring === "monthly"
        ? recurring
        : undefined;
    return {
        isRecurring: task.isRecurring === true || recurring !== null,
        recurringFrequency,
        recurringDays: explicitRecurringDays ??
        ...
        recurringCustomText: existingCustomText ??
            (recurring === "yearly"
                ? "yearly"
                : weeklyDayLabel
...
exports.resolveParsedRecurringTodoFields = resolveParsedRecurringTodoFields;
const formatDateKey = (date) => (0, date_fns_1.format)(date, "yyyy-MM-dd");
const stripToken = (source, token) => source.replace(token, " ").replace(/\s+/g, " ").trim();
const extractField = (source, markerRegex, options) => {
    const match = markerRegex.exec(source);
    if (!match) {
        return { value: null, nextInput: source };
    }
    ...
    if (nextMarkerIndex >= 0)
        valueEnd = Math.min(valueEnd, nextMarkerIndex);
    if (dateBoundaryIndex >= 0)
        valueEnd = Math.min(valueEnd, dateBoundaryIndex);
...
exports.parseMinutesValue = parseMinutesValue;
const addDaysUtc = (date, amount) => {
    const next = new Date(date);
    next.setUTCDate(next.getUTCDate() + amount);
    return next;
};
const getNextMonthlyOccurrenceUtc = (referenceDate, dayOfMonth) => {
...
const detectFallbackDate = (input, referenceDateKey) => {
    const normalized = input.toLowerCase();
    ...
    const monthlyOrdinalMatch = normalized.match(/\b(?:on\s+)?the\s+(\d{1,2})(?:st|nd|rd|th)\s+of\s+(?:every|each)\s+month\b|\b(?:every|each)\s+month\s+on\s+the\s+(\d{1,2})(?:st|nd|rd|th)\b|\b(?:every|each)\s+(\d{1,2})(?:st|nd|rd|th)\s+of\s+the\s+month\b/);
...
exports.detectFallbackDate = detectFallbackDate;
const detectFallbackTime = (input) => {
    const normalized = input.toLowerCase();
...
exports.detectFallbackTime = detectFallbackTime;
const detectRecurringPattern = (input) => {
    const normalized = input.toLowerCase();
    const monthlyOrdinalMatch = normalized.match(/\b(?:on\s+)?the\s+(\d{1,2})(?:st|nd|rd|th)\s+of\s+(?:every|each)\s+month\b|\b(?:every|each)\s+month\s+on\s+the\s+(\d{1,2})(?:st|nd|rd|th)\b|\b(?:every|each)\s+(\d{1,2})(?:st|nd|rd|th)\s+of\s+the\s+month\b/);
    if (monthlyOrdinalMatch) {
        const matchedDay = monthlyOrdinalMatch[1] ?? monthlyOrdinalMatch[2] ?? monthlyOrdinalMatch[3];
    ...
    const weeklyWeekdayMatch = normalized.match(/\b(?:every|each)\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/);
    if (weeklyWeekdayMatch) {
        return {
            recurring: "weekly",
            recurringDay: normalizeWeekdayIndex(weeklyWeekdayMatch[1]),
            recurringDays: normalizeWeekdayIndex(weeklyWeekdayMatch[1]) !== null
                ? [normalizeWeekdayIndex(weeklyWeekdayMatch[1])]
                : undefined,
        };
    }
...
exports.detectRecurringPattern = detectRecurringPattern;
const detectImplicitTaskPriority = (input) => {
    return DUE_TODAY_PRIORITY_PATTERN.test(input) ? "important" : undefined;
};
exports.detectImplicitTaskPriority = detectImplicitTaskPriority;
const extractExplicitSignals = (rawInput) => {
    ...
    return {
        ...
        explicit: {
            ...
            duration: (0, exports.parseMinutesValue)(durationExplicit.value),
            reminder: (0, exports.parseMinutesValue)(reminderExplicit.value),
...
const cleanShortcutTitle = (title, explicit) => {
    let next = title;
    [
        explicit.phone,
        explicit.url,
...
exports.cleanShortcutTitle = cleanShortcutTitle;
const buildHeuristicParsedTask = (rawInput, referenceDateKey, explicit) => {
    const recurrence = (0, exports.detectRecurringPattern)(rawInput);
    const titleBase = (0, exports.cleanShortcutTitle)((0, exports.extractExplicitSignals)(rawInput).inputForModel || rawInput, explicit);
    ...
    return {
        title,
        date: (0, exports.detectFallbackDate)(rawInput, referenceDateKey) ?? referenceDateKey,
        time: (0, exports.detectFallbackTime)(rawInput),
        priority: (0, exports.detectImplicitTaskPriority)(rawInput) ?? "normal",
...
exports.buildHeuristicParsedTask = buildHeuristicParsedTask;
const normalizeIsoDate = (value) => {
    ...
    return (0, date_fns_1.isValid)(parsed) ? (0, date_fns_1.format)(parsed, "yyyy-MM-dd") : null;
};
