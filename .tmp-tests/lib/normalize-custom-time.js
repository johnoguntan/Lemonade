...
exports.normalizeCustomTime = void 0;
...
const resolveClock = (hour, minute, meridiem) => {
    if (meridiem) {
        if (hour < 1 || hour > 12)
            return null;
...
const DURATION_RE = /^(\d+(?:\.\d+)?)\s*(h|hr|hrs|hour|hours|m|min|mins|minute|minutes)$/i;
const formatDuration = (amount, unit) => {
...
const normalizeCustomTime = (input) => {
...
exports.normalizeCustomTime = normalizeCustomTime;
