...
exports.useLemonadeStore = exports.holidays = exports.reconcileOptimisticTaskOrder = exports.localTaskParser = exports.normalizeCalendarDateKey = exports.parseLocalDateKey = exports.formatLocalDateKey = exports.migratePersistedLemonadeState = exports.ensureFixedLists = exports.ensureFixedTabs = exports.FIXED_LISTS = exports.FIXED_LIST_TABS = exports.LEMONADE_STORAGE_KEY = exports.STORAGE_VERSION = exports.todoMatchesSearchFilters = exports.normalizeTodoPriority = exports.extractTaskNotesMetadata = exports.DEFAULT_TASK_SEARCH_FILTERS = exports.createOptimisticTodoId = void 0;
exports.parseNaturalLanguageTaskInput = parseNaturalLanguageTaskInput;
exports.parseNaturalLanguageTaskEntries = parseNaturalLanguageTaskEntries;
    ...
    setItem: (name, value) => {
        try {
            ...
            if (!storageQuotaWarned && typeof window !== 'undefined') {
                ...
                sonner_1.toast.error("Couldn't save your changes — local storage is full.", {
...
const createOptimisticTodoId = (_scope = "task") => globalThis.crypto.randomUUID();
exports.createOptimisticTodoId = createOptimisticTodoId;
...
const computeReminderScheduleIso = (todo) => {
    ...
    if (!todo.time)
        return null;
    if (typeof todo.reminderOffsetMinutes !== "number" || !Number.isFinite(todo.reminderOffsetMinutes))
        return null;
    if (todo.completed)
        return null;
...
const scheduleReminderForTodo = async (todo) => {
    if (typeof window === "undefined")
        return;
    ...
    try {
        const response = await fetch("/api/notifications/schedule", {
            ...
            body: JSON.stringify({
                ...
                title: todo.text,
                scheduledFor,
                loopRule: typeof todo.reminderLoopRule === "string" && todo.reminderLoopRule.trim()
        ...
        if (typeof data.scheduledNotificationId === "string" && data.scheduledNotificationId) {
            exports.useLemonadeStore.getState().updateCalendarTodo(todo.id, {
...
exports.DEFAULT_TASK_SEARCH_FILTERS = {
    status: "all",
    when: "all",
    priority: "all",
    recurring: "all",
...
const extractLegacyStructuredLink = (notes) => {
    const lines = (notes ?? "")
        .split("\n")
        .map((line) => line.trim())
        .filter(Boolean);
...
const cloneTaskHistorySnapshot = (state) => ({
...
const pushTaskHistory = (state, label) => ({
    taskHistoryPast: [
...
const isStructuredLinkValue = (value) => {
...
const isStructuredPhoneValue = (value) => {
    const normalized = value.trim();
    return /^tel:/i.test(normalized) || /^\+?[\d()\-\s]{6,}$/.test(normalized);
};
const isStructuredUrlValue = (value) => {
    const normalized = value.trim();
    return /^(https?:\/\/|www\.)/i.test(normalized);
};
const extractTaskNotesMetadata = (notes) => {
    const lines = (notes ?? "")
        .split("\n")
        .map((line) => line.trim())
        .filter(Boolean);
...
exports.extractTaskNotesMetadata = extractTaskNotesMetadata;
...
const endOfWeekSunday = (date) => startOfLocalDay(addLocalDays(startOfWeekMonday(date), 6));
const createdAtMatchesFilter = (createdAt, filter, referenceDate) => {
    if (filter === "all")
        return true;
...
exports.normalizeTodoPriority = normalizeTodoPriority;
const taskDateMatchesFilter = (todo, filter, referenceDate) => {
    if (filter === "all")
        return true;
    if (filter === "someday")
    ...
    if (!todo.date)
        return false;
    const today = startOfLocalDay(referenceDate);
    const todayKey = (0, exports.formatLocalDateKey)(today);
    const tomorrowKey = (0, exports.formatLocalDateKey)(addLocalDays(today, 1));
    switch (filter) {
        case "today":
        ...
        case "this-week": {
            ...
            return todo.date >= (0, exports.formatLocalDateKey)(weekStart) && todo.date <= (0, exports.formatLocalDateKey)(weekEnd);
        }
        case "next-week": {
            const nextWeekStart = addLocalDays(startOfWeekMonday(today), 7);
            const nextWeekEnd = addLocalDays(nextWeekStart, 6);
            return todo.date >= (0, exports.formatLocalDateKey)(nextWeekStart) && todo.date <= (0, exports.formatLocalDateKey)(nextWeekEnd);
        }
        case "this-month": {
...
const taskPriorityMatchesFilter = (todo, filter) => {
    if (filter === "all")
        return true;
    const priority = (0, exports.normalizeTodoPriority)(todo.priority);
    switch (filter) {
        case "high":
...
const taskAlertMatchesFilter = (todo, filter) => {
    if (filter === "all")
        return true;
...
const todoMatchesSearchFilters = (todo, options) => {
    const { searchQuery, searchModeActive = false, taskSearchFilters = exports.DEFAULT_TASK_SEARCH_FILTERS, labelFilterIds = [], activeFilterColor = null, showCompleted = true, referenceDate = new Date(), } = options;
    if (searchModeActive) {
        if (taskSearchFilters.status === "done" && !todo.completed)
            return false;
        if (taskSearchFilters.status === "todo" && todo.completed)
            return false;
    }
    ...
    if (searchQuery && !todo.text.toLowerCase().includes(searchQuery.toLowerCase()))
        return false;
    if (labelFilterIds.length > 0 && !todo.labelIds.some((labelId) => labelFilterIds.includes(labelId)))
        return false;
    if (activeFilterColor && todo.color !== activeFilterColor)
        return false;
    if (!taskDateMatchesFilter(todo, taskSearchFilters.when, referenceDate))
        return false;
    if (!taskPriorityMatchesFilter(todo, taskSearchFilters.priority))
        return false;
    if (taskSearchFilters.recurring === "yes" && !todo.isRecurring)
        return false;
    if (taskSearchFilters.recurring === "no" && todo.isRecurring)
        return false;
    if (taskSearchFilters.icon && todo.icon !== taskSearchFilters.icon)
        return false;
    const notesMetadata = (0, exports.extractTaskNotesMetadata)(todo.notes);
    const hasAttachment = notesMetadata.hasAttachment || (todo.attachments?.length ?? 0) > 0;
    if (taskSearchFilters.attachment === "yes" && !hasAttachment)
        return false;
    if (taskSearchFilters.attachment === "no" && hasAttachment)
        return false;
    if (taskSearchFilters.url === "yes" && !notesMetadata.hasUrl)
        return false;
    if (taskSearchFilters.url === "no" && notesMetadata.hasUrl)
        return false;
    if (taskSearchFilters.phone === "yes" && !notesMetadata.hasPhone)
        return false;
    if (taskSearchFilters.phone === "no" && notesMetadata.hasPhone)
        return false;
    if (!taskAlertMatchesFilter(todo, taskSearchFilters.alert))
        return false;
    if (!createdAtMatchesFilter(todo.createdAt, taskSearchFilters.dateAdded, referenceDate))
        return false;
    return true;
};
exports.todoMatchesSearchFilters = todoMatchesSearchFilters;
const normalizeSubtask = (subtask, fallbackParentId) => ({
    id: typeof subtask.id === "string" ? subtask.id : generateId(),
    title: typeof subtask.title === "string"
        ? subtask.title
...
exports.STORAGE_VERSION = 3;
exports.LEMONADE_STORAGE_KEY = "lemonade-storage";
const normalizeTodo = (todo, fallbackCreatedAt) => {
    ...
    return {
        ...
        reminderOffsetMinutes: typeof todo.reminderOffsetMinutes === "number" && Number.isFinite(todo.reminderOffsetMinutes)
            ? todo.reminderOffsetMinutes
            : todo.reminderOffsetMinutes === 0
                ? 0
                : null,
        durationMinutes: typeof todo.durationMinutes === "number" && Number.isFinite(todo.durationMinutes) && todo.durationMinutes > 0
            ? Math.floor(todo.durationMinutes)
            : undefined,
        icon: typeof todo.icon === "string" && todo.icon.trim().length > 0 ? todo.icon : undefined,
        photoDataUrl: typeof todo.photoDataUrl === "string" && todo.photoDataUrl.trim().length > 0
            ? todo.photoDataUrl
            : undefined,
        url: typeof todo.url === "string" && todo.url.trim().length > 0
            ? todo.url.trim()
            : extractLegacyStructuredLink(todo.notes),
        location: typeof todo.location === "string" && todo.location.trim().length > 0
            ? todo.location.trim()
            : undefined,
        recurringInterval: typeof todo.recurringInterval === "number" && Number.isFinite(todo.recurringInterval) && todo.recurringInterval > 1
            ? Math.floor(todo.recurringInterval)
            : undefined,
        recurringCustomText: typeof todo.recurringCustomText === "string" && todo.recurringCustomText.trim().length > 0
        ...
        priority: (0, exports.normalizeTodoPriority)(todo.priority),
...
const normalizeLegacyTodo = (todo, fallbackCreatedAt) => ({
    ...normalizeTodo(todo, fallbackCreatedAt),
    parentId: todo.parentId ?? null,
});
const normalizePersistedPreferences = (preferences, fallbackPreferences) => ({
    ...fallbackPreferences,
    ...preferences,
    columns: preferences?.columns === 1 ||
    ...
    accentColor: typeof preferences?.accentColor === "string"
    ...
    defaultLabelId: typeof preferences?.defaultLabelId === "string" ? preferences.defaultLabelId : null,
    displayName: typeof preferences?.displayName === "string" && preferences.displayName.trim().length > 0
        ? preferences.displayName
        : fallbackPreferences.displayName,
});
const clampWeekCount = () => 1;
exports.FIXED_LIST_TABS = [
...
const ensureFixedTabs = (tabs) => {
    const byId = new Map(tabs.map((tab) => [tab.id, tab]));
    return exports.FIXED_LIST_TABS.map((tab) => byId.get(tab.id) ?? tab).concat(tabs.filter((tab) => !exports.FIXED_LIST_TABS.some((fixedTab) => fixedTab.id === tab.id)));
};
exports.ensureFixedTabs = ensureFixedTabs;
const ensureFixedLists = (lists) => {
    const existing = new Map(lists.map((list) => [list.id, list]));
    const normalizedExisting = lists.map((list) => list.id === "shopping-returns"
        ? { ...list, type: "shopping-returns", tabId: "shopping-returns-tab" }
        : list);
    const fixedLists = exports.FIXED_LISTS.map((list) => {
    ...
    const combined = fixedLists.concat(normalizedExisting.filter((list) => !exports.FIXED_LISTS.some((fixedList) => fixedList.id === list.id)));
    ...
    const canonicalPlanningByName = new Map();
    exports.FIXED_LISTS.filter((list) => list.tabId === "planning-tab").forEach((list) => {
        const key = list.name.trim().toUpperCase();
        const current = canonicalPlanningByName.get(key) ?? new Set();
        current.add(list.id);
...
exports.ensureFixedLists = ensureFixedLists;
const ensureUniqueTodoIds = (todos) => {
...
const migratePersistedLemonadeState = (persistedState, fallbackState) => {
    ...
    return {
        ...
        calendarTodos: Array.isArray(persisted.calendarTodos)
            ? ensureUniqueTodoIds(persisted.calendarTodos.map((todo, index) => isLegacyVersion ? normalizeLegacyTodo(todo, index) : normalizeTodo(todo, index)))
            : [],
        listTabs: (0, exports.ensureFixedTabs)(Array.isArray(persisted.listTabs) ? persisted.listTabs : []),
        lists: (0, exports.ensureFixedLists)(Array.isArray(persisted.lists)
            ? persisted.lists.map((list, listIndex) => ({
                ...list,
                todos: Array.isArray(list.todos)
                    ? ensureUniqueTodoIds(list.todos.map((todo, todoIndex) => isLegacyVersion
                        ? normalizeLegacyTodo(todo, listIndex * 1000 + todoIndex)
                        : normalizeTodo(todo, listIndex * 1000 + todoIndex)))
                    : [],
            }))
            : []),
        labels: Array.isArray(persisted.labels)
            ? persisted.labels
            : Array.isArray(persisted.tags)
                ? persisted.tags
                : [],
        searchQuery: typeof persisted.searchQuery === "string" ? persisted.searchQuery : "",
        labelFilterIds: Array.isArray(persisted.labelFilterIds)
            ? persisted.labelFilterIds
            : typeof persisted.tagFilterId === "string"
                ? [persisted.tagFilterId]
                : [],
        collapsedSubtasks: persisted && typeof persisted === "object" && persisted.collapsedSubtasks && typeof persisted.collapsedSubtasks === "object"
            ? Object.fromEntries(Object.entries(persisted.collapsedSubtasks).filter(([, value]) => typeof value === "boolean"))
            : {},
        sidebarOpen: typeof persisted.sidebarOpen === "boolean" ? persisted.sidebarOpen : false,
        weekCount: clampWeekCount(),
        lastSessionDate: typeof persisted.lastSessionDate === "string"
            ? persisted.lastSessionDate
            : null,
        isCalendarExpanded: typeof persisted.isCalendarExpanded === "boolean"
            ? persisted.isCalendarExpanded
            : fallbackState?.isCalendarExpanded ?? false,
    };
};
exports.migratePersistedLemonadeState = migratePersistedLemonadeState;
...
exports.parseLocalDateKey = parseLocalDateKey;
const normalizeCalendarDateKey = (value) => {
    ...
    const parsed = (0, date_fns_1.parseISO)(trimmed);
    if (!(0, date_fns_1.isValid)(parsed)) {
        return undefined;
    }
    return (0, date_fns_1.format)(parsed, "yyyy-MM-dd");
};
exports.normalizeCalendarDateKey = normalizeCalendarDateKey;
const toIsoDate = (date) => (0, exports.formatLocalDateKey)(date);
const normalizeTodoDate = (value) => {
    if (value === null) {
        return null;
    }
    const normalized = (0, exports.normalizeCalendarDateKey)(value);
    if (!normalized) {
        return toIsoDate(new Date());
    }
    return normalized;
};
const getInitialCalendarDate = (startOnYesterday) => {
    const date = new Date();
    if (startOnYesterday) {
        date.setDate(date.getDate() - 1);
    }
    return (0, exports.formatLocalDateKey)(date);
};
const normalizeTextSizePreference = (value) => {
...
const normalizeSpacingPreference = (value) => {
...
const normalizeTimelineHourPreference = (value, fallback) => {
...
const cleanLabelName = (value) => value
    .trim()
    .replace(/^#/, "")
    .replace(/[.!?;:]+$/g, "")
    .trim();
...
const ACTION_START_PATTERN = /^(?:call|run|send|email|text|buy|book|schedule|plan|review|write|read|clean|organize|pick|drop|submit|prepare|get|make|go|meet|pay|finish|start|stop|take|bring|file|draft|return|visit|exercise|work|check|update|follow|talk|message|shop)\b/i;
...
const BLOCKED_TASK_PATTERN = /\b(?:can't|cant|cannot|unable|blocked|stuck|problem|issue|error|broken|won't|wont|doesn't|doesnt|not\s+(?:working|opening|loading|available)|weird\s+file\s+type)\b/i;
const DEADLINE_TASK_PATTERN = /\b(?:asap|urgent|critical|right away|due|deadline|before|by|no later than|end of day|eod|close of business|cob)\b/i;
...
const splitStrongTaskFragments = (input) => {
...
const splitNaturalLanguageTaskFragments = (input) => {
...
const hasExplicitDateOverride = (fragment) => {
    const normalized = fragment.toLowerCase();
    return (/\b(?:today|tomorrow|tmr|next week)\b/.test(normalized) ||
...
const hasOverlap = (ranges, start, end) => ranges.some((range) => start < range.end && end > range.start);
...
const getNextWeekdayDate = (referenceDate, weekday, includeNextWeek = false) => {
    ...
    return (0, date_fns_1.addDays)(base, daysUntil);
};
const detectFastPathScheduledDate = (input, referenceDate) => {
    ...
    if (explicitNextWeekdayMatch) {
        return toIsoDate(getNextWeekdayDate(referenceDate, WEEKDAY_INDEX_BY_NAME[explicitNextWeekdayMatch[1]], true));
    }
    if (/\b(?:tomorrow|tmr)\b/.test(normalized)) {
        return toIsoDate((0, date_fns_1.addDays)(referenceDate, 1));
    }
...
const detectImplicitPriority = (input) => {
...
const stripTokenRanges = (input, ranges) => {
...
function parseNaturalLanguageTaskInput(input, options) {
    ...
    const addParsedLabel = (rawName, index) => {
    ...
    for (const match of taskInput.matchAll(/\b(p1|p2|p3|high priority|medium priority|low priority|urgent|asap|important|normal priority|normal|whenever)\b/gi)) {
        ...
        const value = normalized === "p1" || normalized === "urgent" || normalized === "asap" || normalized === "important" || normalized === "high priority"
            ? "urgent"
...
exports.localTaskParser = parseNaturalLanguageTaskInput;
function parseNaturalLanguageTaskEntries(input, options) {
    const referenceDate = options.referenceDate ?? new Date();
...
const tokenizeTaskText = (value) => value
    .toLowerCase()
    .split(/[^a-z0-9]+/i)
    .filter(Boolean);
const scoreOptimisticTaskMatch = (candidate, aiTask) => {
    const aiTitle = typeof aiTask.title === "string" ? aiTask.title.trim() : "";
    const aiDate = (0, exports.normalizeCalendarDateKey)(typeof aiTask.date === "string" ? aiTask.date : undefined);
...
const reconcileOptimisticTaskOrder = (optimisticTasks, aiTasks) => {
...
exports.reconcileOptimisticTaskOrder = reconcileOptimisticTaskOrder;
const removeRecurringChildren = (todos, parentId) => todos.filter((todo) => todo.parentId !== parentId);
const removeIncompleteRecurringChildren = (todos, parentId) => todos.filter((todo) => todo.parentId !== parentId || todo.completed);
const cloneRecurringSubtasks = (subtasks, parentId) => subtasks.map((subtask) => ({
    ...subtask,
    id: generateId(),
    parentId,
    completed: false,
}));
const getRecurringInterval = (todo) => typeof todo.recurringInterval === "number" && Number.isFinite(todo.recurringInterval) && todo.recurringInterval > 1
    ? Math.floor(todo.recurringInterval)
    : 1;
const differenceInWholeWeeks = (left, right) => {
    const MS_PER_DAY = 24 * 60 * 60 * 1000;
    return Math.floor((startOfDay(left).getTime() - startOfDay(right).getTime()) / (7 * MS_PER_DAY));
};
const insertTodoAfter = (todos, targetId, nextTodo) => {
...
const getTodoSortBucket = (todo) => {
    if (!todo.completed && !todo.endOfDay)
        return 0;
    if (!todo.completed && todo.endOfDay)
        return 1;
    if (todo.completed && !todo.endOfDay)
...
const getSnoozeDate = (option, referenceDate) => {
    ...
    switch (option) {
        ...
        case "this-weekend": {
            ...
            return (0, date_fns_1.addDays)(today, daysUntilSaturday);
        }
        case "next-week": {
            const day = today.getDay();
            const daysUntilNextMonday = ((8 - day) % 7) || 7;
            return (0, date_fns_1.addDays)(today, daysUntilNextMonday);
        }
        default:
            return today;
    }
};
const getNextRecurringOccurrenceDate = (todo, referenceDate) => {
    const interval = getRecurringInterval(todo);
    const base = startOfDay(referenceDate);
    if (todo.recurringFrequency === "daily") {
        return (0, date_fns_1.addDays)(base, interval);
    }
    if (todo.recurringFrequency === "weekday") {
        let offset = 1;
        while (offset < 14 * interval) {
            const candidate = (0, date_fns_1.addDays)(base, offset);
    ...
    if (todo.recurringFrequency === "monthly") {
        return (0, date_fns_1.addMonths)(base, interval);
    }
    ...
    return (0, date_fns_1.addDays)(base, 7);
};
const mapTodosInStore = (state, todoId, updater) => ({
...
exports.useLemonadeStore = (0, zustand_1.create)()((0, middleware_1.persist)((set, get) => ({
    // Default preferences
    preferences: {
        columns: 7,
        textSize: 'md',
    ...
    canRedoTaskAction: false,
    applyTaskIdMap: (idMap) => set((state) => {
        const mapId = (id) => (id ? idMap[id] ?? id : id);
        const remapSubtasks = (subtasks, parentId) => subtasks.map((subtask) => ({
            ...subtask,
            parentId,
        }));
        const remapTodos = (todos) => todos.map((todo) => {
            const nextId = mapId(todo.id);
        ...
        const remapCollapsed = (collapsed) => {
            const entries = Object.entries(collapsed).map(([key, value]) => [mapId(key), value]);
            return Object.fromEntries(entries);
        };
        const remapHistory = (entry) => ({
            ...entry,
            snapshot: {
                calendarTodos: remapTodos(entry.snapshot.calendarTodos),
                collapsedSubtasks: remapCollapsed(entry.snapshot.collapsedSubtasks),
                selectedTaskIds: entry.snapshot.selectedTaskIds.map((id) => mapId(id)),
            },
        });
        return {
            calendarTodos: remapTodos(state.calendarTodos),
            collapsedSubtasks: remapCollapsed(state.collapsedSubtasks),
            selectedTaskIds: state.selectedTaskIds.map((id) => mapId(id)),
    ...
    addCalendarTodo: (todo) => {
        ...
        const resolvedLabelIds = Array.isArray(todo.labelIds) && todo.labelIds.length > 0
            ? todo.labelIds
            : defaultLabelId
                ? [defaultLabelId]
                : [];
        ...
        set((state) => ({
            ...
            quickAddSessionTodoIds: state.quickAddSessionActive && !state.quickAddSessionTodoIds.includes(id)
    ...
    clearLastDeleted: (token) => set((state) => {
    ...
    restoreLastDeletedTodo: () => set((state) => {
        if (!state.lastDeleted) {
            return state;
        }
    ...
    updateCalendarTodo: (id, updates) => {
        set((state) => {
            ...
            const updatedTodo = {
                ...
                priority: "priority" in updates ? (0, exports.normalizeTodoPriority)(updates.priority) : targetTodo.priority,
                reminderOffsetMinutes: "reminderOffsetMinutes" in updates
                ...
                location: "location" in updates
                    ? typeof updates.location === "string" && updates.location.trim().length > 0
                        ...
                        : undefined
                    : targetTodo.location,
                scheduledNotificationId: "scheduledNotificationId" in updates
                    ? typeof updates.scheduledNotificationId === "string" && updates.scheduledNotificationId.trim().length > 0
                        ...
                        : null
                    : targetTodo.scheduledNotificationId ?? null,
                photoDataUrl: "photoDataUrl" in updates
            ...
            const shouldResetChildren = !updatedTodo.parentId &&
            ...
            return {
                ...
                calendarTodos: todosWithoutChildren.map((todo) => todo.id === id ? updatedTodo : todo)
            };
        });
        ...
        if ("reminderOffsetMinutes" in updates ||
            "date" in updates ||
            "time" in updates ||
    ...
    updateCalendarTodoInstance: (id, updates) => set((state) => ({
        ...
        calendarTodos: state.calendarTodos.map((todo) => todo.id === id
            ? {
                ...todo,
                ...updates,
                priority: "priority" in updates ? (0, exports.normalizeTodoPriority)(updates.priority) : todo.priority,
                reminderOffsetMinutes: "reminderOffsetMinutes" in updates
                    ? typeof updates.reminderOffsetMinutes === "number" && Number.isFinite(updates.reminderOffsetMinutes)
                        ? updates.reminderOffsetMinutes
                        : null
    ...
    deleteCalendarTodo: (id) => set((state) => {
        const deletedItems = state.calendarTodos.filter((todo) => todo.id === id || todo.parentId === id);
        if (deletedItems.length === 0) {
            return state;
        }
        return {
            ...pushTaskHistory(state, "delete task"),
            calendarTodos: removeRecurringChildren(state.calendarTodos.filter((todo) => todo.id !== id), id),
            lastDeleted: {
                items: deletedItems,
                token: generateId(),
            },
    ...
    toggleCalendarTodo: (id, origin) => {
        ...
        set((state) => {
            const nextTodos = state.calendarTodos.map((todo) => todo.id === id
    ...
    toggleEndOfDay: (id) => set((state) => ({
        ...
        calendarTodos: state.calendarTodos.map((todo) => todo.id === id ? { ...todo, endOfDay: !todo.endOfDay } : todo)
    })),
    ...
    editSubtask: (todoId, subtaskId, title) => set((state) => ({
        ...
        ...mapTodosInStore(state, todoId, (todo) => ({
            ...todo,
            subtasks: todo.subtasks.map((subtask) => subtask.id === subtaskId ? { ...subtask, title } : subtask),
        })),
    })),
    ...
    setSubtasksCollapsed: (todoId, collapsed) => set((state) => ({
        collapsedSubtasks: {
            ...state.collapsedSubtasks,
            [todoId]: collapsed,
        },
    })),
    toggleSubtasksCollapsed: (todoId) => set((state) => ({
        collapsedSubtasks: {
            ...state.collapsedSubtasks,
    ...
    moveTodoToDate: (todoId, newDate) => set((state) => ({
        ...
        calendarTodos: state.calendarTodos.map((todo) => todo.id === todoId ? { ...todo, date: newDate } : todo)
    })),
    moveCalendarTodoToList: (todoId, listId) => set((state) => {
        ...
        const addDaysLocal = (base, amount) => {
            const next = new Date(base);
            next.setDate(next.getDate() + amount);
            return next;
        };
        const formatKey = (date) => (0, exports.formatLocalDateKey)(date);
        const endOfWeekSunday = (base) => {
            const day = base.getDay();
            const diff = (7 - day) % 7;
            return addDaysLocal(base, diff);
        };
        const nextMonday = (base) => {
            const day = base.getDay();
            const daysUntilNextMonday = ((8 - day) % 7) || 7;
            return addDaysLocal(base, daysUntilNextMonday);
        };
        const endOfMonth = (base) => new Date(base.getFullYear(), base.getMonth() + 1, 0);
        const endOfYear = (base) => new Date(base.getFullYear(), 11, 31);
        ...
        return {
            ...
            lists: state.lists.map((list) => list.id === listId ? { ...list, todos: [...list.todos, moved] } : list),
        };
    }),
    ...
    moveListTodoToDate: (sourceListId, todoId, newDate) => {
        ...
        set((current) => ({
            ...
            lists: current.lists.map((list) => list.id === sourceListId
    ...
    snoozeCalendarTodo: (todoId, option) => set((state) => {
        ...
        const snoozeDate = (0, exports.formatLocalDateKey)(getSnoozeDate(option, new Date()));
        const nextTime = option === "later-today"
            ? `${`${Math.min(new Date().getHours() + 2, 23)}`.padStart(2, "0")}:00`
            : undefined;
        return {
            ...pushTaskHistory(state, "snooze task"),
            calendarTodos: state.calendarTodos.map((todo) => todo.id === todoId
    ...
    reorderCalendarTodo: (todoId, targetId, position) => set((state) => {
        ...
        const base = nextBucket.reduce((min, todo) => Math.min(min, todo.createdAt ?? min), Number.POSITIVE_INFINITY) ??
            Date.now();
    ...
    skipRecurringOccurrence: (todoId) => set((state) => {
        ...
        const series = target.parentId
            ...
            : target;
        const referenceDate = (0, exports.parseLocalDateKey)(target.date ?? (0, exports.formatLocalDateKey)(new Date()));
        const nextDate = (0, exports.formatLocalDateKey)(getNextRecurringOccurrenceDate(series, referenceDate));
    ...
    // Lists
    listTabs: exports.FIXED_LIST_TABS,
    addListTab: (name) => set((state) => ({
        listTabs: [...state.listTabs, { id: generateId(), name }]
    })),
    renameListTab: (id, name) => set((state) => ({
        listTabs: state.listTabs.map((tab) => exports.FIXED_LIST_TABS.some((fixedTab) => fixedTab.id === id)
            ? tab
            : tab.id === id
                ? { ...tab, name }
                : tab)
    ...
    deleteList: (id) => set((state) => ({
        lists: exports.FIXED_LISTS.some((list) => list.id === id)
            ? state.lists
            : state.lists.filter((list) => list.id !== id)
    })),
    renameList: (id, name) => set((state) => ({
        lists: state.lists.map((list) => exports.FIXED_LISTS.some((fixedList) => fixedList.id === id)
            ? list
            : list.id === id
                ? { ...list, name }
                : list)
    ...
    reorderList: (tabId, draggedId, targetId) => set((state) => {
        ...
        const groupedLists = state.lists.filter((list) => (list.tabId ?? (list.type === 'planning' ? 'planning-tab' : 'my-lists-tab')) === tabId);
        ...
        let typeIndex = 0;
        return {
            lists: state.lists.map((list) => (list.tabId ?? (list.type === 'planning' ? 'planning-tab' : 'my-lists-tab')) === tabId
                ? reordered[typeIndex++]
                : list)
        };
    }),
    ...
    moveListToTab: (listId, tabId) => set((state) => {
        ...
        const targetIndices = remainingLists.reduce((indices, list, index) => {
            const listTabId = list.tabId ?? (list.type === 'planning' ? 'planning-tab' : 'my-lists-tab');
            if (listTabId === tabId) {
                indices.push(index);
            }
    ...
    taskSearchFilters: exports.DEFAULT_TASK_SEARCH_FILTERS,
    setTaskSearchFilters: (updates) => set((state) => ({
        taskSearchFilters: {
            ...state.taskSearchFilters,
            ...updates,
        },
    })),
    resetTaskSearchFilters: () => set({ taskSearchFilters: exports.DEFAULT_TASK_SEARCH_FILTERS }),
    // Filter
    labelFilterIds: [],
    ...
    ensureLabelIds: (names) => {
        const state = get();
        const resolvedIds = [];
        const existingByName = new Map(state.labels.map((label) => [normalizeTagName(label.name), label]));
    ...
    addLabelToTask: (taskId, labelId) => set((state) => ({
        calendarTodos: state.calendarTodos.map((todo) => todo.id === taskId && !todo.labelIds.includes(labelId)
            ? { ...todo, labelIds: [...todo.labelIds, labelId] }
            : todo),
        lists: state.lists.map((list) => ({
            ...list,
            todos: list.todos.map((todo) => todo.id === taskId && !todo.labelIds.includes(labelId)
                ? { ...todo, labelIds: [...todo.labelIds, labelId] }
                : todo),
        })),
    })),
    removeLabelFromTask: (taskId, labelId) => set((state) => ({
        calendarTodos: state.calendarTodos.map((todo) => todo.id === taskId
            ? { ...todo, labelIds: todo.labelIds.filter((id) => id !== labelId) }
            : todo),
        lists: state.lists.map((list) => ({
            ...list,
            todos: list.todos.map((todo) => todo.id === taskId
                ? { ...todo, labelIds: todo.labelIds.filter((id) => id !== labelId) }
                : todo),
        })),
    })),
    ...
    generateRecurringInstances: () => {
        const state = get();
        const recurringParents = state.calendarTodos.filter((todo) => todo.isRecurring === true && !todo.parentId && typeof todo.date === "string");
        const generationStart = startOfDay(new Date());
        const generationEnd = (0, date_fns_1.addDays)(generationStart, RECURRING_GENERATION_DAYS);
        ...
        for (const parent of recurringParents) {
            ...
            const existingChildDates = new Set(currentTodos
                .filter((todo) => todo.parentId === parent.id)
                .map((todo) => todo.date));
            const parentStartDate = (0, exports.parseLocalDateKey)(parent.date);
            const nextInstances = [];
            ...
            if (parent.recurringFrequency === 'daily') {
                for (let offset = recurringInterval; offset <= RECURRING_GENERATION_DAYS; offset += recurringInterval) {
                    const candidate = (0, date_fns_1.addDays)(parentStartDate, offset);
                    if (candidate > generationEnd)
                        break;
                    if (candidate < generationStart)
                        continue;
                    const candidateDate = (0, exports.formatLocalDateKey)(candidate);
                    if (existingChildDates.has(candidateDate))
                        continue;
                ...
                for (let offset = 1; offset <= RECURRING_GENERATION_DAYS; offset += 1) {
                    ...
                    if (recurringInterval > 1) {
                        ...
                        if (weekdayBlock % recurringInterval !== 0)
                            continue;
                    }
                    const candidateDate = (0, exports.formatLocalDateKey)(candidate);
                    if (existingChildDates.has(candidateDate))
                ...
                for (let offset = 1; offset <= RECURRING_GENERATION_DAYS; offset += 1) {
                    ...
                    if (recurringInterval > 1) {
                        ...
                        if (weeksSinceParent % recurringInterval !== 0)
                            continue;
                    }
                    const candidateDate = (0, exports.formatLocalDateKey)(candidate);
                    if (existingChildDates.has(candidateDate))
                ...
                for (let monthOffset = recurringInterval; monthOffset <= 36; monthOffset += recurringInterval) {
                    const candidate = (0, date_fns_1.addMonths)(parentStartDate, monthOffset);
                    if (candidate > generationEnd)
                        break;
                    if (candidate < generationStart)
                        continue;
    ...
    // Auto-rollover
    autoRollover: () => {
        const todayStr = (0, exports.formatLocalDateKey)(new Date());
    ...
    name: exports.LEMONADE_STORAGE_KEY,
    version: exports.STORAGE_VERSION,
    storage: (0, middleware_1.createJSONStorage)(() => safeLocalStorage),
    partialize: (state) => ({
    ...
    migrate: (persistedState, version) => {
        const normalized = (0, exports.migratePersistedLemonadeState)(persistedState);
        // Version 0 -> 2:
        ...
        if (typeof version !== "number" || version === 0 || version < exports.STORAGE_VERSION) {
            return normalized;
        }
        return normalized;
    },
    merge: (persistedState, currentState) => {
        const persisted = (0, exports.migratePersistedLemonadeState)(persistedState, currentState);
        return {
            ...currentState,
            ...persisted,
