...
exports.downloadTaskIcs = exports.buildTaskIcs = void 0;
...
const buildTaskIcs = (todo) => {
    ...
    if (year && month && day && minutes != null) {
        ...
        const durationMin = typeof todo.durationMinutes === "number" && todo.durationMinutes > 0 ? todo.durationMinutes : 60;
        const end = new Date(start.getTime() + durationMin * 60000);
        lines.push(`DTSTART:${localStamp(start)}`, `DTEND:${localStamp(end)}`);
    }
    else if (year && month && day) {
        const next = new Date(year, month - 1, day + 1);
        lines.push(`DTSTART;VALUE=DATE:${dateOnly(year, month, day)}`, `DTEND;VALUE=DATE:${dateOnly(next.getFullYear(), next.getMonth() + 1, next.getDate())}`);
    }
    if (todo.location)
        lines.push(`LOCATION:${escapeIcs(todo.location)}`);
    if (todo.url)
        lines.push(`URL:${escapeIcs(todo.url)}`);
    if (todo.notes)
        lines.push(`DESCRIPTION:${escapeIcs(todo.notes)}`);
    lines.push("END:VEVENT", "END:VCALENDAR");
    return lines.join("\r\n");
};
exports.buildTaskIcs = buildTaskIcs;
const downloadTaskIcs = (todo) => {
    const ics = (0, exports.buildTaskIcs)(todo);
...
exports.downloadTaskIcs = downloadTaskIcs;
